...
import (
	...
	"github.com/GPA-Gruppo-Progetti-Avanzati-SRL/tpm-chorus/orchestration/xforms/kz/commons"
...
func SetProperties(_ kazaam.Config) func(spec *transform.Config, data []byte) ([]byte, error) {
	return func(spec *transform.Config, data []byte) ([]byte, error) {
		...
		props, err := operators2.GetArrayParam(spec, SpecParamProperties, true)
		if err != nil {
...
func shouldBeSet(data []byte, keys []string, ifMissing bool, criterion operators2.Criterion) (bool, jsonparser.ValueType, error) {
