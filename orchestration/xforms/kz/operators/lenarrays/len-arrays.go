...
func computeAndSetLenOfArray(outData []byte, dst operators2.JsonReference, data []byte, src operators2.JsonReference) ([]byte, error) {
	const semLogContext = "kazaam-len-arrays::compute-len"

	sourceArray, err := operators2.GetJsonArray(data, src)
	if err != nil {
		log.Error().Err(err).Msg(semLogContext)
		return outData, err
	}
...
func computeAndSetLenOfNestedArray(outData []byte, dst operators2.JsonReference, data []byte, src operators2.JsonReference) ([]byte, error) {
	...
	_, err = jsonparser.ArrayEach(rootArray, func(value []byte, dataType jsonparser.ValueType, offset int, errParam error) {
		...
		nestedLen, err := operators2.LenOfArray(rootArray, nestedRef)
		if err != nil {
			log.Error().Err(err).Msg(semLogContext)
			loopErr = err
			return
