...
func processArray(data []byte, sourceRef operators2.JsonReference, itemKTransformation *kazaam.Kazaam, params OperatorParams) ([]byte, error) {
	const semLogContext = OperatorSemLogContext + "::process-array"

	sourceArray, err := operators2.GetJsonArray(data, sourceRef)
	if err != nil {
		log.Error().Err(err).Msg(semLogContext)
		return nil, err
	}

	lenOfSourceArray, err := operators2.LenOfArray(sourceArray, operators2.JsonReference{})
	if err != nil {
		log.Error().Err(err).Msg(semLogContext)
		return nil, err
	}
	...
	_, err = jsonparser.ArrayEach(sourceArray, func(value []byte, dataType jsonparser.ValueType, offset int, errParam error) {
		...
		if len(params.criteria) > 0 {
			accepted, err = params.criteria.IsAccepted(value, map[string]interface{}{operators2.CriterionSystemVariableKazaamArrayLen: lenOfSourceArray, operators2.CriterionSystemVariableKazaamArrayLoopIndex: arrayItemNdx}) // isAccepted(value, params.criteria)
		}
		if accepted {
